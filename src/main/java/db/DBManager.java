...
//            }
    ...
    public static void ModifyTerm(String id, String duration, String[] disciplines) {
        ...
        try {
            ...
            stmt.execute("DELETE FROM `term_discipline` WHERE (`id` = '" + id + "');\n");
    ...
    public static ArrayList<Discipline> getAllActiveDisciplineBYTerm(String idTerm) {
    ...
    public static ArrayList<Mark> getMarks(String idStud, String idTerm) {
        ...
        try {
            ...
            if (marks.isEmpty()) {
                rs = stmt.executeQuery("SELECT d.id, d.discipline, m.mark FROM mark as m\n" +
                        "left join term_discipline as td on m.id_term_discipline = td.id\n" +
                        "left join discipline as d on td.id_discipline = d.id\n" +
                        "where m.id_student = " + idTerm + " and td.id_term = '1'");
                while (rs.next()) {
                    Mark mark = new Mark();
                    mark.setMark(-1);
                    Discipline discipline = new Discipline();
